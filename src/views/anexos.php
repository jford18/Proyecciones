...
<div class="card shadow-sm"><div class="card-body">
  <form class="row g-2" method="get">
    ...
    <div class="col-md-2"><label class="form-label">Proyecto</label><select name="proyectoId" class="form-select"><option value="">Todos</option><?php foreach ($projectOptions as $project): ?><?php $id = (int) $project['ID']; ?><option value="<?= $id ?>" <?= ((string) ($filters['proyectoId'] ?? '') === (string) $id) ? 'selected' : '' ?>><?= htmlspecialchars((string) $project['NOMBRE']) ?> (<?= $id ?>)</option><?php endforeach; ?></select></div>
