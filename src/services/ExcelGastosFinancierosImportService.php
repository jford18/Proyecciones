...
class ExcelGastosFinancierosImportService
{
    private const SHEET_NAME = '4.- Gastos financieros';
    ...
    public function previewGrid(string $fileTmpPath, string $tipo, ?int $anioRequest = null, ?string $originalFileName = null): array
    {
        ...
        return [
            ...
            'rows' => array_map(static fn (array $row): array => [
                ...
                'DIC' => $row['dic'],
                'TOTAL' => $row['total'],
                'TOTAL_RECALCULADO' => $row['total_recalculado'] ?? $row['total'],
            ], $parsed['rows']),
            'counts' => $parsed['counts'],
            'details' => $parsed['details'],
        ];
    ...
    public function execute(string $fileTmpPath, string $tipo, string $usuario, ?int $anioRequest = null, ?string $originalFileName = null): array
    {
        ...
        try {
            ...
            $upsert = $this->repository->upsertGastosFinancierosRows($tipo, $sheetName, $fileName, $usuario, $rows);
            ...
            $response = [
                ...
                'tab' => 'gastos_financieros',
                'tipo' => $tipo,
                'target_table' => 'PRESUPUESTO_GASTOS_FINANCIEROS',
                'file_name' => $fileName,
    ...
    private function parseGastosFinancieros(string $fileTmpPath, ?int $anioRequest = null, ?string $originalFileName = null): array
    {
        ...
        if (!$sheet instanceof Worksheet) {
            throw new \RuntimeException('No existe la hoja requerida: 4.- Gastos financieros');
        }
        ...
        for ($rowNum = $headerInfo['row'] + 1; $rowNum <= $highestRow; $rowNum++) {
            ...
            $item['total_recalculado'] = $item['total'];
    ...
    {
        $relativePath = 'var/import_store/gastos_financieros.json';
