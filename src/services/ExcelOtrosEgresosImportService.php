...
class ExcelOtrosEgresosImportService
{
    private const TAB = 'otros_egresos';
    private const SHEET_NAME = '6.- Otros egresos';
    private const TARGET_TABLE = 'PRESUPUESTO_OTROS_EGRESOS';
    private const GRID_HEADERS = ['PERIODO', 'CODIGO', 'NOMBRE_CUENTA', 'ENE', 'FEB', 'MAR', 'ABR', 'MAY', 'JUN', 'JUL', 'AGO', 'SEP', 'OCT', 'NOV', 'DIC', 'TOTAL', 'TOTAL_RECALCULADO'];
    ...
    public function execute(string $fileTmpPath, string $tipo, string $usuario, ?int $anioRequest = null, ?string $originalFileName = null): array
    {
        ...
        $counts = ['total_rows' => 0, 'imported_rows' => 0, 'updated_rows' => 0, 'importable_rows' => 0, 'omitted_rows' => 0, 'warning_rows' => 0, 'error_rows' => 0];
        $details = [];
        $rows = [];
        $jsonPath = null;
        ...
        try {
            ...
            $upsert = $this->repository->upsertOtrosEgresosRows($tipo, $sheetName, $fileName, $usuario, $rows);
    ...
    private function parseOtrosEgresos(string $fileTmpPath, ?int $anioRequest = null, ?string $originalFileName = null): array
    {
        ...
        for ($rowNum = $headerInfo['row'] + 1; $rowNum <= $highestRow; $rowNum++) {
            ...
            foreach (self::MONTH_KEYS as $monthKey) {
                ...
                $parsed = $this->readNumericNullable($sheet, $rowNum, $columnIndex, true, $details);
            ...
            $excelTotal = $this->readNumericNullable($sheet, $rowNum, $headerInfo['map']['total'] ?? null, false, $details);
            $totalRecalculado = $hasMonthColumns ? round($sum, 2) : ($excelTotal !== null ? round((float) $excelTotal, 2) : 0.0);
            $item['total'] = $excelTotal;
            $item['total_recalculado'] = $totalRecalculado;
    ...
    private function readNumericNullable(Worksheet $sheet, int $rowNum, ?int $columnIndex, bool $registerWarnings, array &$details): ?float
    {
        if ($columnIndex === null) {
            return null;
        }
        ...
        $rawTrimmed = is_scalar($rawValue) ? trim((string) $rawValue) : '';
        if ($rawTrimmed === '') {
            return null;
        }

        if (is_string($rawValue) && str_starts_with($rawTrimmed, '=')) {
            try {
        ...
        return $parsed['is_numeric'] ? round((float) $parsed['value'], 2) : null;
    }
    ...
    {
        $relativePath = 'var/import_store/otros_egresos.json';
