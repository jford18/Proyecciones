...
class ExcelGastosOperacionalesImportService
{
    private const SHEET_NAME = '3.- Gastos operacionales';
    ...
    {
        $parsed = $this->parseGastosOperacionales($fileTmpPath, $anioRequest, $originalFileName, $clienteSeleccionado ?? '');

        return [
            'ok' => true,
            'tab' => 'gastos_operacionales',
            'tipo' => $tipo,
    ...
    public function execute(string $fileTmpPath, string $tipo, string $usuario, ?int $anioRequest = null, ?string $originalFileName = null, ?string $clienteSeleccionado = null): array
    {
        ...
        try {
            ...
            $upsert = $this->repository->upsertGastosOperacionalesRows($tipo, $sheetName, $fileName, $usuario, $rows);
            ...
            $response = [
                ...
                'tab' => 'gastos_operacionales',
                'tipo' => $tipo,
                'target_table' => 'PRESUPUESTO_GASTOS_OPERACIONALES',
                'file_name' => $fileName,
    ...
    private function parseGastosOperacionales(string $fileTmpPath, ?int $anioRequest = null, ?string $originalFileName = null, ?string $clienteSeleccionado = null): array
    {
        $reader = new Xlsx();
        ...
        if (!$sheet instanceof Worksheet) {
            throw new \RuntimeException('No existe la hoja requerida: 3.- Gastos operacionales');
        }
    ...
    {
        $relativePath = 'var/import_store/gastos_operacionales.json';
